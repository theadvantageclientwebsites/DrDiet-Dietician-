...
const patientDetailKey = (id: string) => ['admin', 'patient', id] as const
...
function ModalContent({ data }: { data: AdminPatientDetail }) {
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Section title="Account">
        ...
        <InfoRow label="Patient ID" value={data.id}         mono />
        <InfoRow label="Joined"     value={formatDate(data.createdAt)} />
        <InfoRow label="Last Updated" value={formatDate(data.updatedAt)} />
      </Section>

      <Section title="Personal Info">
        <InfoRow label="Gender"   value={normalizeGender(profile?.gender)} />
        <InfoRow label="Age"      value={profile?.age != null ? `${profile.age} yrs` : null} />
        <InfoRow label="Height"   value={profile?.heightCm != null ? `${profile.heightCm} cm` : null} />
        <InfoRow label="Weight"   value={profile?.weightKg != null ? `${profile.weightKg} kg` : null} />
        <InfoRow label="Location" value={profile?.location} />
        <InfoRow label="Social"   value={profile?.socialHandle} />
      </Section>

      <Section title="Contact">
        <InfoRow label="Phone"     value={profile?.phoneNumber}    mono />
        <InfoRow label="WhatsApp"  value={profile?.whatsappNumber} mono />
      </Section>
    </div>
  )
}
...
export default function PatientDetailModal({ patientId, onClose }: PatientDetailModalProps) {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: patientDetailKey(patientId ?? ''),
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="pd-scroll" style={{ overflowY: 'auto', flex: 1 }}>
            ...
            {!isLoading && !isError && patient && <ModalContent data={patient} />}

            {!isLoading && !isError && !patient && (
              <div style={{ padding: '40px 20px', textAlign: 'center' }}>
                <p style={{ fontSize: FONT_SIZE.sm, color: COLORS.muted, margin: 0 }}>Patient not found.</p>
              </div>
            )}
          </div>
        </div>
