...
import { useCreateIntern, useUpdateIntern } from '@/hooks/useAdminInternMutations'
...
const EMPTY_FORM: FormState = {
  fullName: '', email: '', phoneNumber: '', universityName: '',
  specialization: '', semester: '', year: '',
...
export default function InternFormModal({ internId, mode, onClose }: InternFormModalProps) {
  const isOpen = mode === 'create' || !!internId
  const [form, setForm] = useState<FormState>(EMPTY_FORM)
  const [generatedPwd, setGeneratedPwd] = useState<string | null>(null)
  const [copied, setCopied] = useState(false)

  const { data, isLoading } = useQuery({
    queryKey: ['admin', 'intern', internId ?? ''],
    queryFn:  () => adminService.getInternById(internId!),
    enabled:  mode === 'edit' && !!internId,
    staleTime: 1000 * 60 * 2,
  })
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      semester:       pr?.semester != null ? String(pr.semester) : '',
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !createMutation.isPending && !updateMutation.isPending && !generatedPwd) onClose()
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [isOpen, onClose, generatedPwd])

  const createMutation = useCreateIntern((pwd) => {
    setGeneratedPwd(pwd)
  })

  const updateMutation = useUpdateIntern(onClose)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (mode === 'create') {
      const payload: AdminInternCreatePayload = {
      ...
      const payload: AdminInternUpdatePayload = {
        fullName:      form.fullName || undefined,
        email:         form.email || undefined,
        accountStatus: (form.accountStatus || undefined) as AdminInternUpdatePayload['accountStatus'],
        internProfile: {
          phoneNumber:    form.phoneNumber || undefined,
          universityName: form.universityName || undefined,
          specialization: form.specialization || undefined,
      ...
      updateMutation.mutate({ id: internId, payload })
    }
  }
      ...
      >
        ...
        >
          ...
          <p style={{ margin: '0 0 20px', fontSize: FONT_SIZE.sm, color: COLORS.muted }}>
            Please save the auto-generated password below and share it with the intern.
          </p>
          <div style={{
  ...
  return (
    <>
      <style>{`
        ...
        .if-field-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
        @media (max-width: 480px) { .if-field-grid { grid-template-columns: 1fr; } }
      `}</style>
      ...
      >
        ...
        >
          <div style={{
            ...
            <div>
              <p style={{ margin: 0, fontSize: FONT_SIZE.base, fontWeight: FONT_WEIGHT.semibold, color: COLORS.navy }}>
                {mode === 'create' ? 'Add New Intern' : 'Edit Intern'}
              </p>
          ...
          <div className="if-scroll" style={{ overflowY: 'auto', flex: 1, padding: '20px' }}>
            {isLoading && mode === 'edit' ? (
              ...
              <form id="intern-form" onSubmit={handleSubmit}>
                ...
                <div className="if-field-grid" style={{ marginBottom: '20px' }}>
                  <FormField id="if-name"  label="Full Name" value={form.fullName}  onChange={(e) => set('fullName', e.target.value)}  placeholder="Jane Intern" required />
                  <FormField id="if-email" label="Email"     value={form.email}     onChange={(e) => set('email', e.target.value)}     placeholder="jane@uni.edu" type="email" required />
                  <div style={{ gridColumn: '1 / -1' }}>
                    <FormField id="if-phone" label="Phone Number" value={form.phoneNumber} onChange={(e) => set('phoneNumber', e.target.value)} type="tel" placeholder="+91 00000 00000" />
                  </div>
                </div>
                ...
                <div className="if-field-grid" style={{ marginBottom: '20px' }}>
                  <div style={{ gridColumn: '1 / -1' }}>
                    <FormField id="if-uni" label="University / Institute" value={form.universityName} onChange={(e) => set('universityName', e.target.value)} placeholder="eg. AIIMS Delhi" />
                  </div>
                  <div style={{ gridColumn: '1 / -1' }}>
                    <FormField id="if-spec" label="Specialization" value={form.specialization} onChange={(e) => set('specialization', e.target.value)} placeholder="eg. M.Sc Dietetics" />
                  </div>
                  <FormField id="if-sem"  label="Semester" value={form.semester} onChange={(e) => set('semester', e.target.value)} type="number" placeholder="4" min="0" max="12" />
                  <FormField id="if-year" label="Year"     value={form.year}     onChange={(e) => set('year', e.target.value)}     type="number" placeholder="2024" min="1900" max="2100" />
                </div>
                ...
                {mode === 'edit' && (
                  <>
                    ...
                    <div className="if-field-grid" style={{ marginBottom: '20px' }}>
                      <SelectField id="if-status" label="Account Status" options={STATUS_OPTS} value={form.accountStatus} onChange={(v) => set('accountStatus', v)} />
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', justifyContent: 'flex-end' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          />
                          <label htmlFor="if-approved" style={{ fontSize: FONT_SIZE.sm, color: COLORS.body, cursor: 'pointer' }}>
                            Approved
                          </label>
                        </div>
                      </div>
          ...
          <div style={{
            ...
            <AdminBtn form="intern-form" type="submit" disabled={isPending || (mode === 'edit' && isLoading)}>
              {isPending ? (mode === 'create' ? 'Creating…' : 'Saving…') : (mode === 'create' ? 'Create Intern' : 'Save Changes')}
            </AdminBtn>
          </div>
        </div>
      </div>
