...
import { BLOOD_GROUPS, BLOOD_GROUP_LABELS, GENDERS } from '@/config/constants'
import { C } from '@/config/colors'
import { useRegisterPatient } from '@/hooks/useAuth'
import type { GenderEnum, BloodGroupEnum } from '@/types'
...
export default function PatientRegisterPage() {
  ...
  const onSubmit: SubmitHandler<FD> = (d) => {
    registerPatient.mutate({
      ...
      bloodGroup: d.bloodGroup as BloodGroupEnum,
    })
  }
    ...
    <FormPageShell
      ...
      footer={
        <div className="flex items-center gap-3">
          ...
          <button form="pt-form" type="submit" disabled={registerPatient.isPending}
            ...
            {registerPatient.isPending ? 'Registering…' : 'Complete Registration'}
          </button>
        </div>
      }
    >
      <form id="pt-form" onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FormField
              id="fullName" label="Full Name" placeholder="eg. John Doe"
            ...
            <Controller name="gender" control={control} render={({ field }) => (
              <SelectField
                ...
                placeholder="Select gender" error={errors.gender?.message}
              />
            )} />
            <div className="sm:col-span-2">
              <FormField
                id="location" label="Location" placeholder="eg. Downtown, New York"
            ...
            />
            <FormField
              id="whatsappNumber" label="WhatsApp Number" type="tel" placeholder="+91 000-000-0000"
              error={errors.whatsappNumber?.message} {...register('whatsappNumber')}
            />
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-[13px] mb-4 -mt-2 leading-relaxed" style={{ color: C.muted }}>
            Health statistics for accurate diet planning.
          </p>
          <div className="grid grid-cols-3 gap-4 mb-4">
            {[
              { id: 'age',      label: 'Age',    unit: 'Yrs', reg: register('age'),      err: errors.age?.message },
              { id: 'heightCm', label: 'Height', unit: 'Cm',  reg: register('heightCm'), err: errors.heightCm?.message },
              { id: 'weightKg', label: 'Weight', unit: 'Kg',  reg: register('weightKg'), err: errors.weightKg?.message },
              ...
              <div key={id} className="flex flex-col gap-1.5">
                ...
                <div className="flex gap-1.5">
                  ...
                  <div className="h-[52px] w-12 flex items-center justify-center rounded-xl border text-[12px] font-medium shrink-0"
                    style={{ background: C.inputBg, borderColor: C.inputBorder, color: C.mutedLight }}>
                    {unit}
                  </div>
                </div>
          ...
          <div className="max-w-[200px]">
            <Controller name="bloodGroup" control={control} render={({ field }) => (
              <SelectField
                ...
                value={field.value} onChange={field.onChange}
                placeholder="Select" error={errors.bloodGroup?.message}
              />
            )} />
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-[13px] mb-4 -mt-2 leading-relaxed" style={{ color: C.muted }}>
            Additional information for specialised care.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
            <FormField
              id="socialHandle" label="Instagram / LinkedIn Handle"
              placeholder="@username" {...register('socialHandle')}
            />
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-semibold" style={{ color: C.navy }}>Defence Personnel</label>
              <Controller name="isDefencePersonnel" control={control} render={({ field }) => (
                <button
                  type="button"
